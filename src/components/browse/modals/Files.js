  ...
  openReport = async () => {
    ...
    await images.map(image => {
      image_data.push([
        image.name,
        image.date,
        image.data.find(datum => datum.name === "Yield Percentage (%)").value,
        image.data.find(datum => datum.name === "Days before Harvest").value
      ]);
      return null;
    });
    ...
    var pdf = {
      info: {
        title: (this.state.season === "WET" ? "WS" : "DS") + this.state.year
      },
      header: {
        width: "40",
        image:
        ...
        { text: "Analytics", style: "subheader" },
        {
          ...
          table: {
            ...
            body: [
              ...
              ["Year", { text: this.state.year, fontSize: 11 }],
              [
                "Pix4D QR",
